/**
 * OrdemProducaoFinanceiroPDF.tsx
 * Componente responsável pela geração do PDF de ORDEM DE PRODUÇÃO (OP) PARA O FINANCEIRO.
 * Contém colunas de Valores que a OP normal não tem.
 */
...
const styles = StyleSheet.create({
  ...
  colIndex: { width: "5%", alignItems: "center", justifyContent: "center", paddingLeft: 4 },
  colImg: { width: "7%", alignItems: "center", justifyContent: "center" },
  colDesc: { width: "52%", paddingLeft: 10, paddingRight: 5, justifyContent: "center" },
  colDescHeader: { width: "52%", paddingLeft: 10 },
  colQty: { width: "8%", textAlign: "center" },
  colUnit: { width: "14%", textAlign: "right", paddingRight: 5, justifyContent: "center" },
  colTotal: { width: "14%", textAlign: "right", paddingRight: 5, justifyContent: "center" },
...
// ─── Componente principal: OrdemProducaoFinanceiroPDF ────────────────────────────────

export const OrdemProducaoFinanceiroPDF = ({ dados }: { dados: DadosImpressao }) => {
  const vendedor = Array.isArray(dados.orcamento.vendedores)
    ? dados.orcamento.vendedores[0]
    : dados.orcamento.vendedores;
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.table}>
          <View style={styles.tableHeader} fixed>
            ...
            <View style={styles.colTotal}>
              <Text style={styles.tableHeaderText}>V. Total</Text>
            </View>
          </View>
          ...
          {dados.itens?.map((item: ItemOrcamento, index: number) => {
            ...
            return (
              <View style={styles.tableRow} key={index} wrap={false}>
                ...
                <View style={styles.colUnit}>
                  <Text style={styles.tableCell}>{formatarMoeda(item.valor_unitario_aplicado || 0)}</Text>
                </View>
                <View style={styles.colTotal}>
                  <Text style={styles.tableCell}>{formatarMoeda(item.subtotal || 0)}</Text>
                </View>
              </View>
            );
          })}
        ...
        {/* Tabela de totais (exclusivo Financeiro) */}
        <View style={{ flexDirection: "row", justifyContent: "flex-end", marginBottom: 20 }}>
          <View style={{ width: "40%", padding: 10, backgroundColor: "#f9fafb", borderRadius: 6, borderWidth: 1, borderColor: "#e5e7eb" }}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 4 }}>
              <Text style={{ fontSize: 10, fontWeight: "bold", color: "#374151" }}>Subtotal Itens:</Text>
              <Text style={{ fontSize: 10, color: "#374151" }}>{formatarMoeda(dados.orcamento.valor_total)}</Text>
            </View>
            <View style={{ flexDirection: "row", justifyContent: "space-between", paddingTop: 4, borderTopWidth: 1, borderTopColor: "#d1d5db" }}>
              <Text style={{ fontSize: 12, fontWeight: "bold", color: "#111827" }}>TOTAL GERAL:</Text>
              <Text style={{ fontSize: 12, fontWeight: "bold", color: "#2563eb" }}>{formatarMoeda(dados.orcamento.valor_total)}</Text>
            </View>
          </View>
        </View>
